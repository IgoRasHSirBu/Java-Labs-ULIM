...
class FisierStudenti {
	...
	void addDateInFisier() throws IOException {
		...
		if (fileIsAssigned) {
			...
			FileWriter fileWritter = new FileWriter(dateInFile, true);// true pentru a face append la continutul deja
																		// existent in fisier
	...
	void prelucrareDateDinFisierV1() throws IOException {// afisarea studentilor din chisinau V1(cu folosirea metodei
															// contains
		...
		if (fileIsAssigned) {
			...
			while ((line = fileReaderBuffer.readLine()) != null) {
				...
				for (int i = 0; i < linelower.length() - 8; i++) {
					if ((linelower.charAt(i) == 'c') && (linelower.charAt(i + 1) == 'h')
							&& (linelower.charAt(i + 2) == 'i') && (linelower.charAt(i + 3) == 'ș')
							|| (linelower.charAt(i + 3) == 's') && (linelower.charAt(i + 4) == 'i')
									&& (linelower.charAt(i + 5) == 'n') && (linelower.charAt(i + 6) == 'ă')
							|| (linelower.charAt(i + 6) == 'a') && (linelower.charAt(i + 7) == 'u')) {
						System.out.println(line);
					}
				}
			}
